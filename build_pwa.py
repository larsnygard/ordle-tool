...
/* pattern row */
.prow{{display:flex;gap:8px;margin-bottom:1rem}}
.pcell{{flex:1;height:58px;background:#1a1a1b;border:2px solid #3a3a3c;
        border-radius:6px;text-align:center;font-size:1.6rem;
        font-weight:700;color:#fff;text-transform:uppercase;
        caret-color:#538d4e}}
.pcell:focus{{outline:none;border-color:#538d4e}}
/* submit */
...
<div id="s-check" class="sec on">
  <label>Mønster &mdash; bruk . for ukjent bokstav</label>
  <div class="prow">
    <input class="pcell" id="p0" maxlength="1" autocomplete="off" autocorrect="off" spellcheck="false">
    <input class="pcell" id="p1" maxlength="1" autocomplete="off" autocorrect="off" spellcheck="false">
    <input class="pcell" id="p2" maxlength="1" autocomplete="off" autocorrect="off" spellcheck="false">
    <input class="pcell" id="p3" maxlength="1" autocomplete="off" autocorrect="off" spellcheck="false">
    <input class="pcell" id="p4" maxlength="1" autocomplete="off" autocorrect="off" spellcheck="false">
  </div>
...
// ── pattern cells: auto-advance ──
for (let i=0;i<5;i++) {{
  const el=document.getElementById('p'+i);
  el.addEventListener('input',()=>{{
    el.value=el.value.replace(/[^a-zA-Z\u00e6\u00f8\u00e5\u00c6\u00d8\u00c5.]/g,'');
    if(el.value && i<4) document.getElementById('p'+(i+1)).focus();
  }});
...
function getPattern(){{
  return [0,1,2,3,4].map(i=>{{
    const v=document.getElementById('p'+i).value.toLowerCase();
    return (v&&v!=='.')?v:'.';
  }}).join('');
}}
...
['ci','ce','p0','p1','p2','p3','p4'].forEach(id=>{{
